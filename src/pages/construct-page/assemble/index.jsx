...
function Assemble() {
    ...
    useEffect(()=>{
        ...
        async function fetchAssembleData(){
            await YottaAPI.getAssembleByName(currentSubjectDomain.domain,currentTopic,1).then(res=>{
                setassembles(res)
            })
        }
        fetchAssembleData();
    ...
    useEffect(() => {
        if (assembles && currentTopic && assembles[currentTopic]) {
            setassnum(assembles[currentTopic].length);
        }
             ...
             <Card  title="碎片" style={assembleStyle}>
                {
                    ...
                    assembles && assembles[currentTopic] && currentTopic? (
                         assembles[currentTopic].map(
                                (assemble)=>
                                   (
